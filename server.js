...
app.post('/apply-subtitles', async (req, res) => {
    try {
        const {
            ...
            include_subtitles: includeSubtitles = true // Default to true
        } = req.body;
        ...
        if (includeSubtitles) {
            ...
            // Step 3: Generate and apply subtitles
            ...
            // Generate the ASS file from the provided content
            ...
            ffmpeg(downloadPath)
                ...
                .on('end', () => {
                    // Step 5: Generate the final video URL for display or download
                    const videoUrl = `${req.protocol}://${req.get('host')}/output/${videoId}.mp4`;
                    res.setHeader('Content-Disposition', `attachment; filename="${videoId}.mp4"`);  // Force download
                    res.json({ videoUrl });
            ...
            // If no subtitles are included, just copy the input file to output
            fs.copyFile(downloadPath, videoFile, (err) => {
                if (err) {
                    return res.status(500).json({ error: 'Failed to copy video without subtitles', details: err.message });
                }
                // Step 5: Generate the final video URL for display or download
                const videoUrl = `${req.protocol}://${req.get('host')}/output/${videoId}.mp4`;
                res.setHeader('Content-Disposition', `attachment; filename="${videoId}.mp4"`);  // Force download
