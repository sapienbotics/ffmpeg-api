...
const effects = [
   ...
   `zoompan=z='if(eq(on,0),1.2,if(gte(zoom,1),zoom-0.0015,zoom))':x='if(gte(on,1),x+1,x)':y='if(gte(on,1),ih/2-(ih/zoom/2)+(x/iw)*0.25,ih/2-(ih/zoom/2))':d=${duration * 30},scale=${width}:${height}:force_original_aspect_ratio=decrease,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2:color=${dominantColor}`,
