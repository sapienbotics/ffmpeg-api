...
const effects = [
    ...
    `zoompan=z='if(gte(zoom,0.8),zoom-0.005,zoom)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${duration * 30}:s=${width}x${height}`,
    ...
    `zoompan=z='if(gte(on,1),zoom+0.005,zoom)':x='if(gte(on,1),x-3,x)':y='ih/2-(ih/zoom/2)':d=${duration * 30}:s=${width}x${height}`,
