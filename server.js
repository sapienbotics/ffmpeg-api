...
async function convertImageToVideo(imageUrl, duration, resolution, orientation) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // Step 4: Improved Zoom + Pan Left Effect
            const effect = `
                zoompan=z='if(eq(on,0),1,zoom+0.0015)':x='iw/2-(iw/zoom/2)+t*3':y='ih/2-(ih/zoom/2)':
                d=${duration * 30}:s=${width}x${height},
                scale=${width}:${height}:force_original_aspect_ratio=decrease,
                pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2:color=${dominantColor}
            `;

            // Step 5: Apply the effect and convert to video
            ffmpeg()
                .input(finalImagePath)
                .loop(duration)
                .outputOptions('-vf', effect.trim())
